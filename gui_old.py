...
class GUI():
    def  __init__(self, photo_path, pic_height=1000, pic_width=800):
        #创建初始基站坐标点
        self.baseA = {'x':200, 'y':300, 'angle':0}
        self.baseB = {'x':500, 'y':400, 'angle':0}
        #创建初始窗口
        self.window = tk.Tk()
        ...
        #将基站所在的位置用椭圆和坐标表示出
        self.plot_oval(self.baseA)
        self.plot_oval(self.baseB)
        #描绘出基站天线的初始方向(朝右边)
    ...
    def plot_oval(self,base,color='pink'): #画出相应点的椭圆和坐标
        oval=self.canvas.create_oval((base['x']-10, base['y']-10, base['x']+10, base['y']+10),fill=color)
        index=self.canvas.create_text(base['x']+25, base['y']+25, text="(x:{} y:{})".format(base['x'],base['y'])
                        , font=("Purisa", 20))
        return oval,index
    ...
    def point_display(self): #RUN按钮触发的函数
        ...
        theta1= self.get_actual_base(base=self.baseA,file_path=file_path.format('A'))
        theta2= self.get_actual_base(base=self.baseB,file_path=file_path.format('B'))
        #得到两条直线的斜率
        k1=tan(theta1)
        k2=tan(theta2)
